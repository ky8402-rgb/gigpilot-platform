...
import { GitOpsLogViewer } from './GitOpsLogViewer';
import {
  ActivityLogItem,
  fetchActivityLogs,
  clearActivityLogs
...
export const ActivityLogsView: React.FC<ActivityLogsViewProps> = ({ onNavigateToTab }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'stream' | 'gitops' | 'explorer' | 'security'>('stream');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-16">
      ...
      <div className="border-b border-slate-800 bg-slate-950/90 px-4 lg:px-8 py-2.5">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            >
              <GitPullRequest className={`w-3.5 h-3.5 ${activeSubTab === 'gitops' ? 'text-white' : 'text-sky-400'}`} />
              <span>GitOps Webhook Sync</span>
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-sky-500/20 text-sky-200 font-bold">
                CI/CD
              </span>
            </button>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-6 space-y-6">
        ...
        {/* SUBTAB 4: GITOPS WEBHOOK SYNCHRONIZATION LOG VIEWER */}
        {activeSubTab === 'gitops' && (
          <GitOpsLogViewer onNavigateToTab={onNavigateToTab} />
        )}
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-xl p-3.5 space-y-3">
          <div className="flex flex-col md:flex-row gap-3">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 text-xs">
              ...
              {['ALL', 'GitHub GitOps', 'Upwork', 'Freelancer', 'RemoteOK', 'Indian Bank', 'PayPal', 'PostgreSQL Backup'].map(src => (
                <button
                  key={src}
