...
export const RealIncomeHub: React.FC<RealIncomeHubProps> = ({
  ...
  // Constants
  const PAYPAL_HANDLE = 'Configured server-side';
  const PAYPAL_EMAIL = 'Configured server-side';
  const ACCOUNT_HOLDER = 'Kundan Kumar';
  const BANK_NAME = 'Configured Payoneer account';
  const BANK_ADDRESS = 'Configured server-side';
  const ROUTING_ABA = 'Configured server-side';
  const ACCOUNT_NUMBER = 'Configured server-side';
  const ACCOUNT_TYPE = 'CHECKING';
  const SWIFT_CODE = 'Configured server-side';
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-3 min-w-[260px] bg-slate-950/70 border border-slate-800 rounded-2xl p-4 text-xs font-mono">
            ...
            <div className="flex items-center justify-between">
              <span className="text-slate-400 flex items-center gap-1.5 font-sans">
                <Building className="w-3.5 h-3.5 text-teal-400" /> Payoneer (Configured Payoneer account):
              </span>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 space-y-2">
              ...
              <p className="text-xs text-slate-300">
                Bill private clients directly via your verified Payoneer Configured Payoneer account wire or custom PayPal.me link with 0% platform intermediary commission.
              </p>
              <button
                onClick={() => setActiveSubTab('custom_link')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md overflow-y-auto">
          <div className="relative w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900 p-6 sm:p-8 shadow-2xl overflow-hidden my-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              >
                🏦 Payoneer Configured Payoneer account (USD)
              </button>
            ...
            {checkoutPaymentMethod === 'wire' && (
              <div className="space-y-3 rounded-2xl bg-slate-950 p-5 border border-slate-800 text-xs">
                <div className="font-bold text-slate-200 border-b border-slate-800 pb-2 flex items-center justify-between">
                  <span>Payoneer USD Checking Account (Configured Payoneer account NY)</span>
